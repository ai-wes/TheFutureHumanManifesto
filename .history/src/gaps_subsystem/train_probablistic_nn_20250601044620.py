...
def train_model(model, train_loader, val_loader, optimizer, criterion, num_epochs, device, patience=15, min_delta=1e-4) -> Dict[str, Any]:
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=max(2, patience//4), min_lr=1e-7, verbose=False) # Changed min_lr, set verbose
