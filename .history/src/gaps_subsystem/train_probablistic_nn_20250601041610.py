...
def train_model(model, train_loader, val_loader, optimizer, criterion, num_epochs, device, patience=15, min_delta=1e-4):
    """
    Trains the model with early stopping and a ReduceLROnPlateau scheduler.
    Args:
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=max(2, patience//4), verbose=True, min_lr=1e-6)
    ...
    if stopped_early and best_model_state is not None:
        ...
        model.load_state_dict(best_model_state)
        logger.info("Model weights set to best validation loss epoch (final epoch was best or no early stop).")
...
if __name__ == "__main__":
    ...
    if len(train_loader) > 0:
        train_model(model, train_loader, val_loader, optimizer, criterion, NUM_EPOCHS, device, patience=15, min_delta=1e-4)
        try:
