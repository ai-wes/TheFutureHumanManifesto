...
if __name__ == "__main__":
    ...
    # 3. If still not enough data, fall back to dummy data generator (if DUMMY_DATA_SIZE > 0)
    if not all_genomes or len(all_genomes) < BATCH_SIZE * 2:
        needed_data_type = "LLM-refined or historical" if not all_genomes else "additional"
        logger.warning(f"Not enough usable {needed_data_type} data (total {len(all_genomes)}).")
        if DUMMY_DATA_SIZE > 0:
            logger.warning(f"Falling back to generating {DUMMY_DATA_SIZE} dummy data scenarios.")
            dummy_genomes, dummy_targets = generate_dummy_scenario_data(DUMMY_DATA_SIZE)
            if dummy_genomes:
                all_genomes.extend(dummy_genomes)
                all_targets.extend(dummy_targets) # Make sure to extend targets
        else:
            logger.warning("DUMMY_DATA_SIZE is 0, no dummy data will be generated.")
