...
# This needs to be consistent with the rest of your project.
# Assuming it's a Pydantic model as per your other scripts.
try:
    from src.utils.models import ScenarioGenome # Preferred location
except ImportError:
    logger.warning("Could not import ScenarioGenome from src.utils.models, using a local definition for now.")
    # Fallback Pydantic model definition (ensure fields match your actual model)
...
MODEL_SAVE_PATH = forecaster_config.get("model_save_path", r"F:\TheFutureHumanManifesto\models\probabilistic_nn.pth")
SCALER_SAVE_PATH = forecaster_config.get("scaler_save_path", r"F:\TheFutureHumanManifesto\models\feature_scaler.joblib")
VECTORIZER_SAVE_PATH = forecaster_config.get("vectorizer_save_path", r"F:\TheFutureHumanManifesto\models\tfidf_vectorizer.joblib")
BAYESIAN_MODEL_SAVE_PATH = forecaster_config.get("bayesian_model_save_path", r"F:\TheFutureHumanManifesto\models\bayesian_ridge_model.joblib")

NN_INPUT_DIM = forecaster_config.get("nn_input_dim", 50)
TFIDF_MAX_FEATURES = forecaster_config.get("tfidf_max_features", NN_INPUT_DIM - 5)
...
DUMMY_DATA_SIZE = training_config.get("dummy_data_size", 1000) # Fallback if no other data

# Paths for the new data sources
HISTORICAL_DATA_PATH = training_config.get("historical_data_path", r"F:\TheFutureHumanManifesto\data\historical_predictions.json")
REFINED_DATA_PATH = training_config.get("refined_data_with_plausibility_path", r"F:\TheFutureHumanManifesto\data\refined_briefs_with_plausibility.json")
ORIGINAL_SYNTHETIC_DATA_PATH = training_config.get("original_synthetic_scenarios_path", r"F:\TheFutureHumanManifesto\data\synthetic_scenarios_generated.json")
