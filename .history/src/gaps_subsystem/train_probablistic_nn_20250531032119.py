...
if __name__ == "__main__":
    ...
    temp_config_loader = ConfigLoader(config_path=os.getenv("GAPS_CONFIG_PATH", r"F:\TheFutureHumanManifesto\config\config.yaml"))
    print("--- Loaded Configuration for Training Script ---")
    print(f"Using config file: {temp_config_loader.config_path}")
    print(f"NN Input Dim from config: {temp_config_loader.get('gapse_settings.forecaster.nn_input_dim')}")
    print(f"TFIDF Max Features from config: {temp_config_loader.get('gapse_settings.forecaster.tfidf_max_features')}")
    print(f"Learning Rate from config: {temp_config_loader.get('gapse_settings.training.learning_rate')}")
    print("--------------------------------------------")
