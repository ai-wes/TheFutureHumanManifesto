...
class ScenarioGenome(BaseModel):
    """Represents a scenario as an evolvable genome (LOOSENED: all fields optional/defaults)"""
    id: Optional[str] = None
    technological_factors: Optional[List[str]] = Field(default_factory=list)
    social_factors: Optional[List[str]] = Field(default_factory=list)
    economic_factors: Optional[List[str]] = Field(default_factory=list)
    timeline: Optional[str] = None
    key_events: Optional[List[str]] = Field(default_factory=list)
    domains: Optional[List[Domain]] = Field(default_factory=list)
    probability_weights: Optional[Dict[str, float]] = Field(default_factory=dict)
    fitness_score: Optional[float] = None
    generation: Optional[int] = 0
    parent_ids: Optional[List[str]] = Field(default_factory=list)

    class Config:
        extra = "allow"  # Allow extra fields in messy data
