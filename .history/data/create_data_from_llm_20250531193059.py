...
def main():
    ...
    for i, scenario_obj in enumerate(input_scenarios):
        ...
        # Run both endpoints asynchronously
        llm_outputs = asyncio.run(process_scenario_with_endpoints(system_prompt, scenario_id))
        for idx, raw_llm_output in enumerate(llm_outputs):
            if raw_llm_output:
                ...
                if json_str:
                    try:
                        ...
                        processed_count += 1
                        print(f"  Successfully processed and parsed LLM output for scenario ID: {scenario_id} from endpoint {idx}")

                    except json.JSONDecodeError:
                        print(f"  Error: LLM output for scenario ID {scenario_id} from endpoint {idx} was not valid JSON.")
                        print(f"  Raw LLM Output:\n{raw_llm_output}")
                        with open(f"error_output_{scenario_id}_endpoint_{idx}.txt", "w", encoding="utf-8") as err_f:
                            err_f.write(f"System Prompt:\n{system_prompt}\n\nRaw LLM Output:\n{raw_llm_output}")
                    except Exception as e:
                        print(f"  An unexpected error occurred while processing LLM output for {scenario_id} from endpoint {idx}: {e}")
                else:
                    print(f"  Error: No JSON found in LLM output for scenario ID {scenario_id} from endpoint {idx}.")
                    with open(f"error_output_{scenario_id}_endpoint_{idx}.txt", "w", encoding="utf-8") as err_f:
                        err_f.write(f"System Prompt:\n{system_prompt}\n\nRaw LLM Output:\n{raw_llm_output}")
            else:
                print(f"  Failed to get LLM output for scenario ID: {scenario_id} from endpoint {idx}. Skipping.")
