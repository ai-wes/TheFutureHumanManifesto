...
OUTPUT_JSON_FILE = r"F:\TheFutureHumanManifesto\real_data\historical_refined_scenarios_briefs.json"
...
Original Prediction Date: {prediction_date}

Original Actual Outcome Date: {actual_outcome_date}
...
Unforeseen Influencing Factors(Positive):
{unforeseen_influencing_factors_positive}

Unforeseen Influencing Factors(Negative):
...
async def process_and_refine_scenario(session, api_url, system_prompt_template_text, scenario_obj, scenario_id):
    ...
    raw_llm_output = await get_chat_completion_async(session, api_url, system_prompt_template_text, scenario_id)
