...
{original_key_events_list_as_string}
...
def format_system_prompt(template, scenario_obj):
    ...
    return template.format(
        ...
        end_year=end_year,
        original_key_events_list_as_string=format_list_for_prompt(scenario_obj.get("key_events", [])),
...
ENDPOINTS = [
    ...
    "https://your-second-endpoint.com/v1/chat/completions"
]
...
def main():
    ...
    for scenario_id, raw_llm_output, scenario_obj in results:
        ...
        # Periodic saving
        if processed_count > 0 and (processed_count % SAVE_INTERVAL == 0 or (scenario_id in processed_ids)):
